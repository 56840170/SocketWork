...
{
    public static class WebSocketConverter
    {
        ...
        public static string GetSecKeyAccetp(byte[] handShakeBytes, int bytesLength)
        {
            ...
            if (handShakeText.Contains("Sec-WebSocket-Key"))
            {
        ...
        /// <returns></returns>
        public static (string message, byte[] buffer) AnalyticData(byte[] recBytes, int recByteLength)
        {
            if (recByteLength < 2) { return (string.Empty, null); }

            bool fin = (recBytes[0] & 0x80) == 0x80; // 1bit，1表示最后一帧  
            if (!fin)
            {
                return (string.Empty, null);// 超过一帧暂不处理 
            }

            bool mask_flag = (recBytes[1] & 0x80) == 0x80; // 是否包含掩码  
            if (!mask_flag)
            {
                return (string.Empty, null);// 不包含掩码的暂不处理
            }
            ...
            return (Encoding.UTF8.GetString(payload_data), payload_data);
        }
        ...
        public static byte[] PackData(string message)
        {
            ...
            return PackBuffer(temp);
        }


        public static byte[] PackBuffer(byte[] temp)
        {
